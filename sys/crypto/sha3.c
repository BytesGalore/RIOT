    ...
    for(round=0; round<24; round++) {
        ...
        {   // === ρ and π steps (see [Keccak Reference, Sections 2.3.3 and 2.3.4]) ===
            ...
            for(t=0; t<24; t++) {
                ...
                size_t r = ((t+1)*(t+2)/2)%64;
                // Compute ((0 1)(2 3)) * (x y)
                size_t Y = (2*x+3*y)%5; x = y; y = Y;
        ...
        {   // === ι step (see [Keccak Reference, Section 2.3.5]) ===
            for(j=0; j<7; j++) {
                size_t bitPosition = (1<<j)-1; //2^j-1
...
/**
  ...
  * @param  input_size    The number of input bytes provided in the input message.
  ...
  * @param  output_size   The number of output bytes desired.
  * @pre    One must have r+c=1600 and the rate a multiple of 8 bits in this implementation.
  */
static void Keccak(size_t rate, size_t capacity, const uint8_t *input, size_t input_size, uint8_t delimitedSuffix, uint8_t *output, size_t output_size)
{
    uint8_t state[200];
    ...
    while(input_size > 0) {
        blockSize = MIN(input_size, rateInBytes);
        for(i=0; i<blockSize; i++)
            state[i] ^= input[i];
        input += blockSize;
        input_size -= blockSize;
    ...
    while(output_size > 0) {
        blockSize = MIN(output_size, rateInBytes);
...
/**
  ...
  */
static void FIPS202_SHAKE128(const uint8_t *input, size_t input_size, uint8_t *output, int output_size)
{
    Keccak(1344, 256, input, input_size, 0x1F, output, output_size);
}

/**
  *  Function to compute SHAKE256 on the input message with any output length.
  */
static void FIPS202_SHAKE256(const uint8_t *input, size_t input_size, uint8_t *output, int output_size)
{
    Keccak(1088, 512, input, input_size, 0x1F, output, output_size);
}

/**
  *  Function to compute SHA3-224 on the input message. The output length is fixed to 28 bytes.
  */
static void FIPS202_SHA3_224(const uint8_t *input, size_t input_size, uint8_t *output)
{
    Keccak(1152, 448, input, input_size, 0x06, output, 28);
}

/**
  *  Function to compute SHA3-256 on the input message. The output length is fixed to 32 bytes.
  */
static void FIPS202_SHA3_256(const uint8_t *input, size_t input_size, uint8_t *output)
{
    Keccak(1088, 512, input, input_size, 0x06, output, 32);
}

/**
  *  Function to compute SHA3-384 on the input message. The output length is fixed to 48 bytes.
  */
static void FIPS202_SHA3_384(const uint8_t *input, size_t input_size, uint8_t *output)
{
    Keccak(832, 768, input, input_size, 0x06, output, 48);
}

/**
  *  Function to compute SHA3-512 on the input message. The output length is fixed to 64 bytes.
  */
static void FIPS202_SHA3_512(const uint8_t *input, size_t input_size, uint8_t *output)
{
    Keccak(576, 1024, input, input_size, 0x06, output, 64);
}
#endif
void sha3(size_t digest_length, const uint8_t *input, size_t input_size, uint8_t *output)
{
    Keccak(1600-(2*digest_length), 2*digest_length, input, input_size, 0x06, output, digest_length/8);
}
